...
export default function AuditTable(userInfo) {
	...
	const { value: showMyIncidents, setValue: setShowMyIncidents } = useInput(false);

	const { value: searchedIncidents, setValue: setSearchedIncidents } = useInput('');
				...
				<Navbar
					...
					<Form inline>
						<Form.Check
							...
							onClick={() => {setShowMyIncidents(!showMyIncidents); updateDisplayLog();}}
						/>
						<FormControl 
							type='text' 
							placeholder='Search' 
							className='mr-sm-2'
							value={searchedIncidents} 
							/>
